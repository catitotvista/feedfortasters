...
const inputStyleInput = (focus, error) => ({
...
function FieldInput({ label, hint, error, required, children }) {
  return (
...
export function Input({ label, hint, error, required, style, ...rest }) {
  ...
  return (
    <FieldInput label={label} hint={hint} error={error} required={required}>
      <input onFocus={() => setFocus(true)} onBlur={() => setFocus(false)} style={{ ...inputStyleInput(focus, error), ...style }} {...rest} />
    </FieldInput>
  );
}
